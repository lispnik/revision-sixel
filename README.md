...
(revision-sixel:run)                        ; a single image (the coast photo)
(revision-sixel:run "media/flower.jpg")     ; or any baseline JPEG
```
...
```sh
make bin            # dumps ./revision-sixel-demo (SBCL image, samples baked in)
./revision-sixel-demo               # runs the gallery
./revision-sixel-demo some.jpg …    # or show your own baseline JPEGs
```

`build.lisp` bakes the bundled JPEGs into the saved image (`*embedded-images*`)
and dumps an executable whose toplevel is `revision-sixel:main`. At startup the
...
Key pieces, all in `src/revision-sixel.lisp`:
...
`revision-sixel` depends on `tv2` (→ `tvision`) and `jpeg-sixel`, all sibling
...
```lisp
(asdf:test-system "revision-sixel")
```
